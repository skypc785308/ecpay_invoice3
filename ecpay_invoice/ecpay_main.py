    ...
    *2 - 2驗證參數格式
    ...
    def check_extend_string(self, arParameters = dict):
        ...
        if arParameters['OnLine'] == False:
            if arParameters['CarruerType'] == EcpayCarrierType.No and len(arParameters['CustomerIdentifier']) == 0:
                if arParameters['Print'] != EcpayPrintMark.Yes:
        ...
        if arParameters['CarruerType'] != EcpayCarrierType.No and arParameters['CarruerType'] != EcpayCarrierType.Member and arParameters['CarruerType'] != EcpayCarrierType.Citizen and arParameters['CarruerType'] != EcpayCarrierType.Cellphone:
            arErrors.append("15:Invalid CarruerType.")
        else:
            # * 統一編號不為空字串時，則載具類別不可為會載具或自然人憑證載具
            if len(arParameters['CustomerIdentifier']) > 0:
                if arParameters['CarruerType'] == EcpayCarrierType.Member or arParameters['CarruerType'] == EcpayCarrierType.Citizen:
                    arErrors.append("15:Invalid CarruerType.")

        # 16.載具編號CarruerNum(預設為空字串)
        # * 載具類別為無載具(None)或會員載具(Member)時，請設定空字串
        if arParameters['CarruerType'] == EcpayCarrierType.No or arParameters['CarruerType'] == EcpayCarrierType.Member:
        ...
        elif arParameters['CarruerType'] == EcpayCarrierType.Citizen:
        ...
        elif arParameters['CarruerType'] == EcpayCarrierType.Cellphone:
